...
exports.AppDataSource = new typeorm_1.DataSource({
    ...
    synchronize: true,
    logging: false,
    entities: [Like_1.Like, SavedRecipe_1.SavedRecipe, Comment_1.Comment, Subscription_1.Subscription]
});
async function startRabbitConsumer() {
    await (0, rabbit_1.startConsumer)("social-service.recipe-events", ["recipe.deleted"], async (event) => {
        if (event.type !== "recipe.deleted")
            return;
        const recipeId = Number(event.recipe_id);
        await exports.AppDataSource.getRepository(Like_1.Like).delete({ recipe_id: recipeId });
        await exports.AppDataSource.getRepository(SavedRecipe_1.SavedRecipe).delete({ recipe_id: recipeId });
        await exports.AppDataSource.getRepository(Comment_1.Comment).delete({ recipe_id: recipeId });
