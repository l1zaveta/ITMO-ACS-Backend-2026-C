...
RecipeController.create = async (req, res) => {
    try {
        ...
        if (Array.isArray(rawSteps) && rawSteps.length > 0) {
            const stepRepository = index_1.AppDataSource.getRepository(Step_1.Step);
            const steps = rawSteps.map((step, index) => {
                var _b;
                return stepRepository.create({
                    recipe_id: recipe.recipe_id,
                    step_number: Number((_b = step.step_number) !== null && _b !== void 0 ? _b : index + 1),
        ...
        if (Array.isArray(rawIngredients) && rawIngredients.length > 0) {
            const recipeIngredientRepository = index_1.AppDataSource.getRepository(RecipeIngredient_1.RecipeIngredient);
            const recipeIngredients = rawIngredients.map((ingredient) => {
                var _b;
